...
SessionConflux v2 uses Feishu Drive (not Wiki Docx, not a self-hosted server) as the transport layer for syncing AI agent sessions across machines. Sessions are stored as JSONL compressed archives in a hierarchical folder structure (`{hostname}/baseline/` and `{hostname}/incremental/`). The folder hierarchy itself serves as the index — no separate manifest file. AgentsView continues to read from local SQLite — Feishu is invisible to it.
